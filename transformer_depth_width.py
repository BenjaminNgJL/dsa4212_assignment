...
# narrow models 
model_narrow , params_narrow = create_train_state(key, vocab_size, 128, n_layers, 4, max_len, pos_encoding=pos_encoding)
model_wide , params_wide = create_train_state(key, vocab_size, 512, n_layers, 16, max_len, pos_encoding=pos_encoding)

# shallow models
model_shallow_narrow , params_shallow_narrow = create_train_state(key, vocab_size, 128, 1, 4, max_len, pos_encoding=pos_encoding)
model_shallow_medium , params_shallow_medium = create_train_state(key, vocab_size, 256, 1, 8, max_len, pos_encoding=pos_encoding)
model_shallow_wide , params_shallow_wide = create_train_state(key, vocab_size, 512, 1, 16, max_len, pos_encoding=pos_encoding)

# deep models
model_deep_narrow , params_deep_narrow = create_train_state(key, vocab_size, 128, 4, 4, max_len, pos_encoding=pos_encoding)
model_deep_medium , params_deep_medium = create_train_state(key, vocab_size, 256, 4, 8, max_len, pos_encoding=pos_encoding)
model_deep_wide , params_deep_wide = create_train_state(key, vocab_size, 512, 4, 16, max_len, pos_encoding=pos_encoding)
...
# ============================================================
# TRAINING SETTINGS — THREE MODEL SETS (USING PREDEFINED MODELS)
# ============================================================
B, T = 128, 32
max_time = 180  # seconds per model
results = {}

# ---------- DEFINE 3 MODEL GROUPS ----------
architecture_groups = {
    # 1️⃣ Width comparison (same depth = 2)
    "width_comparison": [
        ("narrow", model_narrow, params_narrow),
        ("baseline", model_baseline, params_baseline),
        ("wide", model_wide, params_wide),
    ],

    # 2️⃣ Shallow comparison (1 layer)
    "shallow_comparison": [
        ("shallow_narrow", model_shallow_narrow, params_shallow_narrow),
        ("shallow_medium",
         create_train_state(key, vocab_size, 256, 1, 8, max_len, pos_encoding=pos_encoding)[0],
         create_train_state(key, vocab_size, 256, 1, 8, max_len, pos_encoding=pos_encoding)[1]),
        ("shallow_wide", model_shallow_wide, params_shallow_wide),
    ],

    # 3️⃣ Deep comparison (4 layers)
    "deep_comparison": [
        ("deep_narrow", model_deep_narrow, params_deep_narrow),
        ("deep_medium",
         create_train_state(key, vocab_size, 256, 4, 8, max_len, pos_encoding=pos_encoding)[0],
         create_train_state(key, vocab_size, 256, 4, 8, max_len, pos_encoding=pos_encoding)[1]),
        ("deep_wide", model_deep_wide, params_deep_wide),
    ],
}

# ---------- TRAIN EACH GROUP ----------
for feature, model_list in architecture_groups.items():
    print(f"\n==============================")
    print(f"🏋️ Training Models — {feature.upper()}")
    print("==============================")
    results[feature] = []
...
# ============================================================
# PLOT TRAINING LOSS FOR EACH GROUP
# ============================================================
...
print("\n✅ Training complete for all 3 architecture groups!")
